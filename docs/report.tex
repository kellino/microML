...
The project's final requirements, in \textit{MoSCoW} form are can be seen in
...
Many errors and omissions were thrown up by the unit-testing which otherwise might not have been
noticed until runtime, or worse, after system deployment. One particular `bug' which was not noticed
until near submission (due to a non-comprehensive test suite) was the inability to perform
mathematics on negative floating point numbers. The parser did not understand the order of
precedence of the symbols. This was easily fixed but would have removed unnoticed without good unit
testing.
